"""
FinSight AI Fund Analysis & ML Prediction Engine.

Loads legacy fund models and dataset from models/fund_model/ directory.
Provides ModelStore singleton for fund recommendations, quality classification,
and wealth planning.
"""
...
# Resolve path to models/fund_model directory
BASE_DIR = Path(__file__).resolve().parents[2]
FUND_MODEL_DIR = BASE_DIR / "models" / "fund_model"
...
class ModelStore:
    ...
    def __init__(self):
        ...
        # Load models and dataset automatically on initialization
        self.load_all()

    def load_all(self):
        print("Loading legacy fund models and dataset...")
        self.regressor = joblib.load(FUND_MODEL_DIR / "returns_3yr_predictor.joblib")
        self.classifier = joblib.load(FUND_MODEL_DIR / "fund_classifier.joblib")
        self.label_encoder = joblib.load(FUND_MODEL_DIR / "fund_label_encoder.joblib")
        self.dataset = pd.read_csv(FUND_MODEL_DIR / "fastapi_funds_dataset.csv")
        ...
        self._loaded = True
        print(f"[fund_service] {len(self.dataset)} funds loaded successfully")
    ...
    def recommend_funds(
        ...
        cat_mask = df["category"].apply(
            lambda x: any(c.lower() in str(x).lower() for c in suitable_categories)
        )
        ...
        clf_proba = self.classifier.predict_proba(filtered[self.clf_features])
        ...
        for category, percentage in active_categories.items():
            ...
            if filtered_by_amt.empty:
                if investment_mode == "SIP":
                    ...
                    filtered_by_amt = cat_df[
                        cat_df["min_lumpsum"] <= allocated_amount
                    ]
                max_funds = 1
            else:
                max_funds = 2
            ...
            cat_df["predicted_return"] = self.regressor.predict(
                cat_df[self.reg_features]
            ).round(2)
